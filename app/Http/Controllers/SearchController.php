...
class SearchController extends Controller
{
		...
		public function searchLocation(Request $request)
    {
       	...
       	$result = Book::where('book_location','LIKE','%'.$request->term.'%')->distinct()->take(7)->get(['book_location']);  
    		foreach ($result as $key => $value) {
    		$searchResult[] = $value->book_location;    	
    	}
      if (empty($searchResult)) {
     return;
      }
    ...
    public function bookSearch(Request $request)
    {
        ...
        $book_author = Book::where('book_author','LIKE','%'.$request->term.'%')->distinct()->take(10)->get();
       	$book_category = Category::where('name','LIKE','%'.$request->term.'%')->distinct()->take(10)->get();
				$book_location = Book::where('book_location','LIKE','%'.$request->term.'%')->distinct()->take(10)->get();
      ...
      foreach ($book_author as $key => $value) {
        $searchResult[] = $value->book_author;
			...
			foreach ($book_location as $key => $value) {
        $searchResult[] = $value->book_location;
      }
